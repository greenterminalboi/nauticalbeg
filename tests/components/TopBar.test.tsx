...
describe("TopBar", () => {
  it("shows all four app sections and the file picker, with no nation selector (decision 2026-09-18: nation picker is Country Viewer-scoped, not global)", () => {
    render(
      <TopBar
    ...
    expect(screen.getByRole("button", { name: /country viewer/i })).toBeInTheDocument();
  ...
  it("marks the active section and still shows the keep toggle regardless of which section is active", () => {
    ...
    expect(screen.getByRole("button", { name: /country viewer/i })).not.toHaveAttribute("aria-current");
