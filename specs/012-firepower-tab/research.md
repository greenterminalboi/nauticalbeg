...
## 9. Post-ship, explicit user request (2026-09-22): "Army Composition" sub-tab and per-unit-type combat stats

**Decision**: A new Firepower sub-view, "Army Composition" — a single-country detail view showing everything ArmyStatsTable's hover breakdowns already carried, but always visible rather than hover-gated. `CountrySelect.tsx` (a new component, single-select searchable combobox mirroring `GoodSelect`'s existing interaction pattern from World Goods) picks the one country; `FirepowerTab.tsx` owns an independent `compositionIdx` state (separate from the multi-select `selectedIdxs` Doctrine/Army/Navy share), so picking a country for Army Composition never disturbs those tabs' own comparison set. Defaults to the human-played nation via the existing `computeDefaultSelection`.

**Regiment Composition combat stats**: the user's ask ("these are easy to pull... they're part of regiment information already") was confirmed correct — `game/in_game/common/unit_categories/*.txt` (12 files, one per raw EU5 category) declares the base combat stats (`max_strength`, `combat_power`, `frontage`, `combat_speed`, `initiative`, `flanking_ability`, `secure_flanks_defense`, `morale_damage_taken`, `strength_damage_taken`, `morale_damage_done`, `strength_damage_done`, `food_storage_per_strength`, `food_consumption_per_strength`, `movement_speed`, `supply_weight` — the full list `unit_categories/readme.txt` documents), and `artillery_barrage` (added on a follow-up request; declared only on the land age-templates' artillery entries, not in that readme). `tools/firepower-reference/generate-unit-types.ts` was extended to resolve every one of these per concrete unit type via the *same* `copy_from`-chain walk it already used for `category`/`age`/`levy` (own field wins; falls back to the unit's raw category's own `unit_categories/*.txt` declaration; falls back to `0`, the engine's own default for an unset modifier, only if genuinely absent at every level — confirmed empirically that ordinary unit types like `a_chambered_cannon` only `copy_from` an age template with no stat overrides, while "unique" unit types, e.g. `D008_byzantine_unit_types.txt`'s, do override a stat directly).

**`unitWeight` is a static multiplier, not the game's live "Unit Weight"**: `game_concepts/00_game_concepts.txt`'s own description says the real in-game "Unit Weight" is `supply_weight` (this reference table's `unitWeight`) multiplied by a regiment's *current* strength — a save-time quantity this static per-unit-type reference table doesn't carry. Surfaced as-is (the static multiplier), not fabricated into a live total this app has no reliable per-regiment strength source to compute.

**Age badge reflects the unit type's own age, never the country's unlocked age**: `UnitTypeReferenceEntry.age` (used for the badge, via `regimentClassifier.ts`'s `UnitTypeTotals.age`) is the *specific unit type's* resolved age tier from its own `copy_from` chain — a completely separate computation from `computeUnlockedAge()` in `armyNavyStats.ts`, which only feeds the country-level `ageArtillery`/`ageInfantry`/`ageCavalry`/`ageSupply` scalars shown in the Equipment Ages section. Confirmed live: a country's Equipment Ages can show e.g. all "III" while individual regiment rows in the same country show a mix of I/II/III (older units still fielded, not yet replaced) — the two are deliberately independent.

**Age + category badges, and column sorting**: both badges are colored (not just text) per Constitution Principle VI still requiring the color never be the *only* signal — each badge's own visible text (roman numeral / category name) carries the information independent of its color. The Regiment Composition table's 20 columns (4 base + 16 stats) reuse the existing `useTableSort`/`SortableHeader` pattern verbatim from `ArmyStatsTable`/`NavyStatsTable`. Column headers are rotated to vertical text (`writing-mode: vertical-rl` + a 180° flip) since 16 stat columns under horizontal headers made the table's width driven by label length rather than by the actual (short, numeric) data underneath.
