...
- [x] T027a [US2] **Add a nation selector (FR-015, added 2026-09-17 per explicit request — "that's going to be a big pattern here")**: updated `spec.md` first — new Acceptance Scenario 3 under User Story 2, new FR-015, and superseded the Assumptions bullet that had explicitly deferred "selecting among multiple human nations" to a future feature
- [x] T027b [US2] Generalized `getPlayerNationOverview` in `src/storage/queries.ts` into `getNationOverview(db, nationIdx)` (by index, for any nation), with `getPlayerNationOverview` now a thin wrapper that looks up the player's idx and delegates; added `listNations(db)` (`WHERE country_type = 'Real'`, per research-save-format.md's country_type note) for the selector's options. Renamed `PlayerNationOverview` → `NationOverview` everywhere (now also carries `idx`) since the type is no longer player-specific. Updated `contracts/data-access-contract.md` to match
- [x] T027c [US2] Created `src/components/Overview/NationSelector.tsx` (+ CSS per design.md's "Text Inputs & Formulary Parchments"): a deliberately generic `{ nations, selectedIdx, onSelect }` dropdown, not nation-specific in shape, since the list+select-by-idx pattern is meant to extend to future selectable views
- [x] T027d [US2] Wired `FileLoader.tsx` to keep one read-only `SaveDatabase` connection open for the life of a "ready" session (previously opened-queried-closed per load) — closed only when superseded by a new load or on unmount — so switching the selected nation re-queries `getNationOverview` against the live connection without re-parsing or re-uploading the save. Added `listNations`/`getNationOverview` tests (`tests/storage/queries.test.ts`) and 2 new `NationSelector` component tests — 40/40 total passing. Manually verified in Chrome: selecting a different real nation (e.g. SCA vs. RUS in the test fixture) re-renders `OverviewCard` with that nation's real stats
...
- [x] T036 [US4] Implement FR-014 in `src/storage/queries.ts`: catch a storage-quota error during `keepSave` and surface it clearly without corrupting the existing kept save or leaving a partially-written database. Done, but the shape changed along the way — see the note below T038. `worker.ts`'s `handleKeep` classifies a caught error as `quotaExceeded` (checks for `DOMException("QuotaExceededError")` or a "quota" substring) and posts a `keep-failed` message with a clear, distinct string ("Not enough storage space is available to keep this save.") rather than a raw error dump. FR-014's "never corrupts the existing kept save" is enforced by ordering: `markSaveKept`'s write happens *before* `recordKeptSave` ever touches the previous kept save (test: `tests/storage/keep-save.test.ts`'s "a failed keepSave write never touches a previously kept save")
- [x] T037 [US4] Create `src/components/Overview/KeepSaveToggle.tsx` letting the user keep (FR-011) or forget (FR-013) the current save. Done, styled per design.md's "Primary Imperial Clasp"/"Wax Seal Stamp" component specs (gold for keep, cinnabar for forget), showing a `[ kept ]` marker and inline pending/error state. 5 component tests
- [x] T038 [US4] On startup, call `listKeptSave()` from `src/app.tsx` and, if one exists, offer to resume it before requiring a fresh upload (Acceptance Scenario 2). **Deviation**: implemented inside `FileLoader.tsx` (a mount effect), not `app.tsx` — this component already owns all save-loading state/lifecycle (worker, `readDbRef`, `beforeunload` cleanup), so splitting the kept-save check into the still-trivial `app.tsx` would mean lifting all of that state up too; `app.tsx` stays a one-line shell. New `KeptSaveOffer.tsx` component (styled per design.md) renders the offer with "Resume This Save" / "Load a Different Save" actions — dismissing does NOT forget the kept save, just lets the user upload something else instead
  - **Major discovery while implementing T036-T038**: writing to a save's OPFS-backed SQLite database (`markSaveKept`'s `UPDATE`) requires a write-capable connection, and *that can only be opened from within a dedicated Worker in this browser* — confirmed by reproducing the exact crash from a plain `openSaveDatabase()` (no readonly flag) call on the main thread: `createSyncAccessHandle is not a function`, thrown even before any write statement runs. This forced `keep` to become a new worker-protocol message (`KeepMessage`/`KeptMessage`/`KeepFailedMessage` — see `contracts/worker-protocol.md`) rather than a direct call from `KeepSaveToggle.tsx`. `forgetKeptSave` is the one exception that stayed a direct main-thread call, since it never opens SQLite at all (pure OPFS-directory + `localStorage` operations).
  - **Second discovery, same session**: `localStorage` doesn't exist inside a Worker's global scope at all (it's `Window`-only) — the original combined `keepSave(db, saveId)` crashed with `localStorage is not defined` the first time it actually ran inside `worker.ts`'s new `handleKeep`. Split into `markSaveKept(db, saveId)` (SQL-only, worker-safe) and `recordKeptSave(summary)` (the `localStorage` pointer, main-thread-only) — see `data-access-contract.md`'s update. `keepSave` still exists, combining both, for same-thread test callers.
  - **Third fix**: `cleanupSaveIfNotKept` wasn't idempotent — `beforeunload` firing it against a saveId the user had *already* explicitly forgotten (its OPFS file already gone) threw an uncaught `SQLiteError` in the browser console. Now treats "can't open" as "nothing to clean up" and returns.
  - Resuming (`resumeSave` in `load-save.ts`) reopens the already-parsed database directly and skips file-reading/version-detection/parsing entirely, reusing the exact `ready`/`error` protocol shape a fresh load produces, so `FileLoader.tsx`'s `handleReady` needed zero changes to also handle a resume.
  - All three discoveries were found and fixed via real manual testing in Chrome (not caught by any unit test, since Node/jsdom has neither a real Worker nor an absent-`localStorage`-in-Worker quirk to catch it) — the full keep → reload → resume → forget → reload round trip was verified end-to-end with a clean console.
- [x] T039 [P] [US4] Write an integration test in `tests/storage/keep-save.test.ts` covering: keep → simulated reload → resume; forget → no longer offered; keeping a second save replaces the first; **and (per T035) that an unkept save's OPFS database is actually deleted on replace and on session end**. Done at the storage layer (8/8 tests, up from 5 — added the FR-014 non-corruption test and the `cleanupSaveIfNotKept` idempotency regression test) — `deleteSaveDatabase` is asserted (via spy) to be called for an unkept save and not for a kept one, and the keep/forget/list pointer roundtrip is verified against real SQLite state. Added `tests/parser/load-save.test.ts`'s `resumeSave` suite (2 tests) and `tests/components/KeepSaveToggle.test.tsx`/`KeptSaveOffer.test.tsx` (7 tests) alongside it. The actual resume-through-UI flow (not just its pieces) was verified manually in Chrome instead of by an automated test — no OPFS in Node/jsdom, same documented gap as T025

**Checkpoint**: All four user stories are independently functional; quickstart.md scenarios 6 and 7 are satisfied. Manually verified end-to-end in Chrome: keep a save, reload the page, get offered to resume it, resume it (same data, no re-upload), forget it, reload again — no longer offered.
...
- [x] T040 [P] Update `ARCHITECTURE.md` to reflect the as-built module boundaries and note any deviations from plan.md/data-model.md. Done — added sections for the two big discoveries from T036-T038 (writes must happen in the Worker; `localStorage` doesn't exist there either), the concurrent-query corruption bug from the OverviewCard session, the FR-015 "pick an entity, view it" generalized pattern, and refreshed the persistence-model section for `markSaveKept`/`recordKeptSave`/resume. Noted `src/domain/` is empty/unused (Principle VII — nothing needed it) and added `src/styles/`, neither of which the original doc mentioned
- [x] T041 [P] Verify `OverviewCard.tsx` and `ErrorMessage.tsx` against constitution Principle VI (text contrast, no color-only meaning) and note the check per the constitution's Development Workflow section. Computed actual WCAG contrast ratios (relative-luminance formula) for every foreground/background pair across all six Overview components (not just the two named), not just eyeballed: everything passed AA except one real failure — white button text on the gold `--color-tertiary` background (`KeepSaveToggle`'s "Keep This Save" and `KeptSaveOffer`'s "Resume This Save" buttons) measured 3.19:1 against a 4.5:1 requirement. Fixed by switching that text to `--color-on-surface` (5.38:1) — which also better matches design.md's own "Primary Imperial Clasp" spec ("iron-gall ink" text, not white). No color-only meaning anywhere: the "computed" badge and error-kind distinctions are both text-based, not color-only. All interactive elements are native `<button>`/`<select>`, confirmed keyboard-operable in Chrome (Tab focus ring visible, Enter activates)
- [x] T042 Run the full quickstart.md validation pass (all 8 scenarios) against a real save file, in both the local dev server and the Docker container (T005/T006). Ran all 8 against the Docker container (`docker compose up -d --build`, verified COOP/COEP headers present) using the real parser pipeline with small fixture-derived saves (scenario-specific variants: a valid save, a second distinct valid save, a non-save text file, a doctored unsupported-version save, and a truncated one — the two full real save files on disk are 84MB/642MB, over this session's browser-automation upload cap, but no scenario here is data-size-sensitive). All 8 passed. **Scenario 8 (cancel mid-parse) found a real bug**: rapid-fire selecting two files back-to-back left an orphaned OPFS database behind. Root cause — `loadSave` created a database (`openSaveDatabase`+`applySchema`) but the abort signal is only checked during file-reading (a known, already-documented T022 gap), not inside `applySchema`/the adapter; for a small/fast file, the "cancelled" load often finishes anyway before noticing the abort, and *also*, independently, any save that fails after database creation (not just a cancelled one) was never cleaned up either, since neither the FR-010 supersede-cleanup nor `beforeunload` cleanup applies to a save that never reached "ready" in the first place. Fixed in `load-save.ts`: `loadSave` now tracks whether it reached `onReady`, and deletes the database in its `finally` block if not. Same bug class as T035, different trigger. Added a regression test (`tests/parser/load-save.test.ts`, forcing a post-creation failure via a spy since a real truncated file actually fails *earlier*, during version detection, before any database exists — jomini needs the whole document well-formed even for a single-field lookup). 57/57 tests passing after the fix; re-verified the same rapid-fire scenario in the (rebuilt) Docker container afterward — exactly one OPFS file remains every time, clean console
