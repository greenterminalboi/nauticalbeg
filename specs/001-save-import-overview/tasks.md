...
- [ ] T027 [US2] Create `src/components/Overview/OverviewCard.tsx` rendering all six stats: nation name, in-game date, total development, province count, treasury, stability, government type, and current war status (FR-006)
- [ ] T028 [US2] Implement a visible marker (e.g., a "computed" badge or footnote) in `OverviewCard.tsx` for stats flagged as derived in the query result, satisfying FR-007
- [ ] T029 [P] [US2] Write a component test in `tests/components/OverviewCard.test.tsx` asserting derived stats carry the visual marker and raw stats do not
